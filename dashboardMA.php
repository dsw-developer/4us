...
<body>
  ...
  <div class="d-flex flex-column p-4 bg-white shadow-lg rounded-4" style="width: 290px; min-height: 50vh;">
    
    <!-- Cabeçalho -->
    <div class="text-center mb-4">
      <h4 class="fw-bold text-orange">Adm 4US</h4>
      <small class="text-muted">Cadastrar Produtos</small>
    </div>

    <!-- Botões Sexo -->
    <div class="btn-group mb-4" role="group" style="border-radius: 12px; overflow: hidden;">
      <button class="btn btn-outline-primary" onclick="window.location.href='dashboard.php'">
        👨 Homem
      </button>
      <button class="btn btn-outline-danger" onclick="window.location.href='dashboardMP.php'">
        👩 Mulher
      </button>
    </div>

    <!-- Navegação Lateral 4us -->
<ul class="nav flex-column">
  <li class="nav-item mb-2">
    <a href="dashboardMP.php" class="nav-link sidebar-link d-flex align-items-center">
      <i class="bi bi-droplet-fill me-3 text-primary fs-5"></i>
      <span>Perfumes e Cosméticos</span>
    </a>
  </li>

  <li class="nav-item mb-2">
    <a href="dashboardMR.php" class="nav-link sidebar-link d-flex align-items-center">
      <i class="bi bi-shirt me-3 text-success fs-5"></i>
      <span>Vestuários</span>
    </a>
  </li>

  <li class="nav-item mb-2">
    <a href="dashboardMT.php" class="nav-link sidebar-link d-flex align-items-center">
      <i class="bi bi-bag-heart-fill me-3 text-warning fs-5"></i>
      <span>Calçados</span>
    </a>
  </li>

  <li class="nav-item mb-2">
    <a href="dashboardMA.php" class="nav-link sidebar-link d-flex align-items-center">
      <i class="bi bi-watch me-3 text-danger fs-5"></i>
  ...
  .nav-link span {
    font-size: 16px;
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    $sql = "INSERT INTO acessorios (name, descricao, preco, img, video, estoque)
                            VALUES ('$name', '$descricao', '$preco', '$caminho_arquivo', '$video', '$estoque')";
...
<body>
  ...
  <form method="POST" action="add_acessoriosM.php" enctype="multipart/form-data" class="product-form">
    <div class="form-group">
        <label for="nome">Nome dos Acessórios de Mulheres</label>
        <input type="text" name="name" id="nome" placeholder="Acessórios de Mulheres" required>
    </div>

    <div class="form-group">
        <label for="descricao">Descrição dos Acessórios de Mulheres</label>
        <textarea name="descricao" id="descricao" placeholder="Descrição dos Acessórios de Mulheres" rows="4" required></textarea>
    </div>

    <div class="form-group">
        <label for="preco">Preço (Kz)</label>
        <input type="number" name="preco" id="preco" placeholder="kz" required step="0.01" min="0">
    </div>
    ...
    <button type="submit" class="btn-submit">Adicionar Produto de Mulher</button>
</form>
      ...
      <div class="row">
          ...
          if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                  ...
                  echo "<img src='" . htmlspecialchars($row['img']) . "' alt='" . htmlspecialchars($row['name']) . "' class='img-fluid'>";
                  echo "<h3>" . htmlspecialchars($row['name']) . "</h3>";
                  echo "<p>" . htmlspecialchars($row['descricao']) . "</p>";
                  echo "<p>Kz " . htmlspecialchars($row['preco']) . "</p>";
                  // Botão de Eliminar
                  echo "<form method='POST' action='delete_acessorioM.php' style='display: inline;'>";
